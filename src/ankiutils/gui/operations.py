...
class AddonQueryOp(QueryOp[T]):
    def without_collection(self) -> AddonQueryOp[T]:
        if has_serialized_ops:
            return cast(AddonQueryOp[T], super().without_collection())
        return self

    def with_progress(self, label: str | None = None) -> AddonQueryOp[T]:
        # with_progress() was broken before Anki 2.1.50, so we avoid calling it
        if point_version() >= 50:
            return cast(AddonQueryOp[T], super().with_progress(label))
        return self
